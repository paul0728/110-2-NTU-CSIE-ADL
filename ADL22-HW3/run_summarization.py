...
from trainer_seq2seq_sum import Seq2SeqTrainer
    ...
    def __post_init__(self):
        if self.dataset_name is None and self.train_file is None and self.validation_file is None and self.test_file is None:
            ...
            if self.train_file is not None:
                ...
                assert extension in ["csv", "json","jsonl"], "`train_file` should be a csv or a json file or a jsonl file."
            if self.validation_file is not None:
                extension = self.validation_file.split(".")[-1]
                assert extension in ["csv", "json","jsonl"], "`validation_file` should be a csv or a json or a jsonl file."
...
def main():
    ...
    def compute_metrics(eval_preds):
        ...
        # preds = [pred + "\n" for pred in preds]
        # labels = [label + "\n" for label in labels]
        # result = metric(preds=preds, refs=labels,ignore_empty=True)
        result = metric(preds=preds, refs=labels,ignore_empty=True)
    ...
    def post_processing_function(examples, pred_outputs, prefix='eval'):
        ...
        prediction_file = data_args.output_file if data_args.output_file is not None else os.path.join(training_args.output_dir, f"{prefix}_predictions.jsonl")
        print("test_file complete")
    ...
    if training_args.do_train:
        ...
        best_ckpt_path=f'{training_args.output_dir}/best_ckpt'
        trainer.save_model(best_ckpt_path)  # Saves the tokenizer too for easy upload
        ...
        import shutil
        trainer.save_state()
        shutil.move(f"./{training_args.output_dir}/trainer_state.json", f"{best_ckpt_path}/trainer_state.json")
