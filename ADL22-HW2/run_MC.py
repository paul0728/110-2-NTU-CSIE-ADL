...
class ModelArguments:
    ...
    train_from_scratch: bool = field(
        default=False,
        metadata={
            "help": "Whether to train a transformer model from scratch"
        },
    )
...
class DataTrainingArguments:
    ...
    test_file: Optional[str] = field(
        default=None,
        metadata={"help": "An optional input testing data file to test the perplexity on (a text file)."},
    )
...
def main():
    ...
    #Load saved test_paragraphs
    with open('./preprocess/mc/test_paragraphs.pickle', 'rb') as f:
...
#         output_json = {'data': []}
        ...
        for i, pred in enumerate(tqdm(preds)):
            ex = {
                ...
                'question': test_dataset['sent1'][i],
                'paragraphs': test_paragraphs[i],
                'relevant': test_paragraphs[i][pred] if pred < len(test_paragraphs[i]) else test_paragraphs[0]
            }
...
#             output_json['data'].append(ex)
            ...
            # endregion
        if data_args.output_file is None:
            data_args.output_file=training_args.output_dir+'prediction.json'
